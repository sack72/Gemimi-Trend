...
def identify_swings_and_fvgs(df):
    ...
    # --- FVGs (Marked at the completion candle, i+2) ---
...
def find_structure_legs(df):
    legs = []

    # --- 1. INITIALIZATION: Find the FIRST valid leg (UP or DOWN) ---
    first_leg = None
        ...
        if fvg_idx != -1:
            ...
            if high_idx != -1:
                if low_idx < first_leg_end_idx:
                    first_leg = {
                        'id': 1, 'Type': 'DOWN',
                        'Start_Time': df.at[high_idx, 'Local time'], 'Start_Price': df.at[high_idx, 'High'], 'Start_Idx': high_idx,
                        'End_Time': df.at[low_idx, 'Local time'], 'End_Price': df.at[low_idx, 'Low'], 'End_Idx': low_idx
                    }
        ...
        if fvg_idx != -1:
            ...
            if low_idx != -1:
                if high_idx < first_leg_end_idx:
                    first_leg = {
                        'id': 1, 'Type': 'UP',
                        'Start_Time': df.at[low_idx, 'Local time'], 'Start_Price': df.at[low_idx, 'Low'], 'Start_Idx': low_idx,
                        'End_Time': df.at[high_idx, 'Local time'], 'End_Price': df.at[high_idx, 'High'], 'End_Idx': high_idx
                    }
    ...
    while search_idx < len(df) - 1:
        ...
        if leg_type == 'DOWN':
            continuation_trigger = current_leg['End_Price'] # Low
            reversal_trigger = current_leg['Start_Price']   # High
        else: # UP
            continuation_trigger = current_leg['End_Price'] # High
            reversal_trigger = current_leg['Start_Price']   # Low
        ...
        for i in range(search_idx + 1, len(df)):
                    ...
                    if start_point_idx != -1:
                        ...
                        if start_point_idx == current_leg['Start_Idx']:
                             ...
                             legs.append({
                                ...
                                'Start_Time': df.at[start_point_idx, 'Local time'], 'Start_Price': df.at[start_point_idx, 'High' if leg_type=='DOWN' else 'Low'], 'Start_Idx': start_point_idx,
                                'End_Time': df.at[swing_idx, 'Local time'], 'End_Price': df.at[swing_idx, 'Low' if leg_type=='DOWN' else 'High'], 'End_Idx': swing_idx
                             })
            ...
            # --- B. CHECK REVERSAL ---
                ...
                for k in range(oc_idx, anchor_idx, -1):
                    if new_type == 'UP' and df.at[k, 'Bullish_FVG']: fvg_idx = k; break # Found most recent
                    if new_type == 'DOWN' and df.at[k, 'Bearish_FVG']: fvg_idx = k; break # Found most recent
                ...
                if fvg_idx != -1:
                    ...
                    if new_anchor_idx != -1:
                        ...
                        legs.append({
                            ...
                            'Start_Time': df.at[new_anchor_idx, 'Local time'], 'Start_Price': df.at[new_anchor_idx, 'Low' if new_type=='UP' else 'High'], 'Start_Idx': new_anchor_idx,
                            'End_Time': df.at[swing_idx, 'Local time'], 'End_Price': df.at[swing_idx, 'High' if new_type=='UP' else 'Low'], 'End_Idx': swing_idx
                        })
                        search_idx = swing_idx; event_found = True

        if not event_found: break

    # 3. Clean Duplicates (Redundant now but good for safety)
...
for fn in uploaded.keys():
    ...
    try:
        ...
        print(f"   ℹ️ Data Loaded. Rows: {len(df)}")

        df_processed = identify_swings_and_fvgs(df)
        legs_df = find_structure_legs(df_processed)

        if not legs_df.empty:
            output_filename = fn.replace(".csv", "_STRUCTURE.csv")
            legs_df.to_csv(output_filename, index=False)
            print(f"✅ Success! Generated {output_filename}")
            print(legs_df[['id', 'Type', 'Start_Time', 'Start_Price', 'End_Time', 'End_Price']].head(15).to_markdown(index=False))
